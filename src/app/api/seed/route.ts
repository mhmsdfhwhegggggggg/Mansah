...
export async function POST(request: NextRequest) {
  ...
  const rateLimitResult = await checkRateLimit(`seed:${ip}`, RATE_LIMITS.seed)
  ...
  try {
    // Create admin user - passwords MUST be provided via environment variables
    const adminPass = process.env.SEED_ADMIN_PASSWORD
    const agentPass = process.env.SEED_AGENT_PASSWORD
    const customerPass = process.env.SEED_CUSTOMER_PASSWORD

    if (!adminPass || !agentPass || !customerPass) {
      return NextResponse.json({ error: 'يجب تعيين متغيرات البيئة لكلمات المرور (SEED_ADMIN_PASSWORD, SEED_AGENT_PASSWORD, SEED_CUSTOMER_PASSWORD)' }, { status: 500 })
    }
    ...
    const sampleProducts = [
      ...
      },
      {
        ...
        sourcePlatform: SourcePlatform.ALIBABA,
        categoryId: categories[0].id,
        rating: 4.3,
        reviewCount: 12500,
        isFeatured: true,
