...
const mobileWallets = [
  { id: 'JAWALI', name: 'محفظة جوالي', account: '77XXXXXXX' },
  { id: 'JEEB', name: 'محفظة جيب', account: '73XXXXXXX' },
  { id: 'ONE_CASH', name: 'ون كاش', account: '71XXXXXXX' },
  { id: 'FLOOSAK', name: 'فلوسك', account: '70XXXXXXX' },
]
...
export default function CheckoutPage() {
  ...
  const [mobileWallet, setMobileWallet] = useState('JAWALI')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Handle specifics for mobile wallet
      if (paymentMethod === 'MOBILE_WALLET') {
        if (!walletPhone) {
          toast.error('يرجى إدخال رقم هاتف المحفظة المحول منها')
          setLoading(false)
          return
        }
        const selectedWallet = mobileWallets.find(w => w.id === mobileWallet)
        paymentPayload.bankName = selectedWallet?.name
        paymentPayload.accountNumber = walletPhone
      }
              ...
              <div className="card p-6">
                ...
                {paymentMethod === 'MOBILE_WALLET' && (
                  <div className="mt-4 p-4 bg-purple-50 rounded-xl">
                    <h4 className="font-bold text-purple-800 text-sm mb-4">اختر المحفظة الإلكترونية:</h4>
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                         {mobileWallets.map(wallet => (
                           <button
                             key={wallet.id}
                             type="button"
                             onClick={() => setMobileWallet(wallet.id)}
                             className={`p-3 rounded-lg border text-right text-sm transition-all focus:outline-none ${
                      ...
                      <div className="bg-white p-4 rounded-lg border border-purple-100">
                         <p className="text-sm text-purple-800 mb-1">يرجى التحويل إلى الحساب التالي:</p>
                         <p className="font-mono font-bold text-xl text-purple-900 border-b pb-3 mb-4">
                           {mobileWallets.find(w => w.id === mobileWallet)?.account}
                         </p>
                         
                         <label className="block text-sm font-medium text-purple-800 mb-1.5">رقم هاتفك (المحول منه) للتأكيد:</label>
                         <input
                           type="tel"
                           value={walletPhone}
                           onChange={(e) => setWalletPhone(e.target.value)}
                           className="w-full px-4 py-2 border border-purple-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-right"
                    ...
                    <p className="text-xs text-purple-600 mt-3">
                      * سيتم التأكد من وصول الحوالة ومطابقة رقم هاتفك المحول منه خلال ساعات
                    </p>
                  </div>
                )}
              </div>
